...
\title{Daily Labor Supply and Unanticipated Shocks: Taxi Drivers' Response to Cancellations and No-shows}
...
is associated with 23\% reduction from the mean hazard rate of stopping work at a given time, and a no-show 22\% reduction. 
The cancellation effect is equivalent to the effects of additional 38 minutes of work, or additional 41 Singapore dollars 
...
These results have important policy implications. Negative shocks are an integral part of our lives: Researchers' articles are rejected by editors, factory workers are unable to work due to machine breakdowns, and taxi drivers' bookings get cancelled by riders. When observing a worker encountering such unanticipated and unwanted events, the firms and the policy makers may want to compensate the worker to make up for their loss. However, if income targeting behavior is present, it is important to recognize that the compensation can have the side effect of reducing the workers' labor supply and own income. From a firm's perspective, this can be detrimental to its profit. From a policy marker's perspective, due to reduced marginal utility of income after reaching the reference point, the perceived value from such a compensation will be smaller when giving to someone who are near the target than giving to someone who are really behind the target. In fact, to maximize the perceived welfare impact of a compensation, it is best not to give it the workers right away as part of their daily income, since not only will it reduce the motivation to work and the workers' own income, but the workers may also even discount its value once they pass the target. Instead, the equivalent compensation should be saved and given out only when the workers encounter extreme shocks or anticipated shocks, during which the compensation will not affect their labor supply and its welfare value will not get discounted.

% We rule out the following alternative explanations. First, C\&NS do not affect nearby drivers, who are subject 
% to the same market conditions, and the results are robust even with hour-day of week by building fixed effects
% and controls for market thickness at different spatial and temporal scale. Thus, unobserved demand 
% and supply factors are unlikely to generate the results. Second, controlling for fatigue due to time spent with passengers and time driving has little impact on the estimated coefficients, and hence, fatigue is also unlikely to explain the C\&NS effects. Third, to rule out the impact of unobserved driver characteristics, such as ability, experience, learning, and plans, we use driver fixed effects, even allowing the driver fixed effects to vary with month, with day of week, and with regions. Furthermore, we observe the C\&NS effects even in regions and bookings where we expect little heterogeneity among the drivers.
...
% We refer to this panel as the ``hour-level data."
...
Specification (5) controls for cumulative income effects. If the income target is fixed, we would expect the realized earnings to soak up all the effects of unanticipated earnings. On the other hand, if the target is endogenous or adaptive, we would not expect the realized earnings to fully capture the effects of unanticipated shocks, since the income targets will change with the anticipated portion of realized earnings, diluting the income effects. The magnitude of the estimated C\&NS effects in Specification (5) are slightly lower than that of Specification (4), but the differences, -1.23 ppts (s.e. 0.10) and -1.16 (s.e. 0.16) versus -1.36 ppts (s.e. 0.10) and -1.31 ppts (s.e. 0.16), are minimal, suggesting that the effects of realized earnings does not fully soak up the effects of unanticipated shocks caused by C\&NS, and hence, the fixed target theory does not fully explain our results. Consequently, our results can be taken as an evidence in favor of an endogenous and/or adaptive reference point over a fixed reference point. 
...
similar to results in previous sections. Row (12) estimates the C\&NS effects for the trips and bookings from central business district (CDB), an area that must be very familiar to all the drivers and in which we expect little heterogeneity in
...
Row (14) to (19) use different bandwidth sizes and weighting schemes to estimate the local weighted regressions. The results are overall robust to the choice of bandwidth and weights. However, the precision of the estimates decreases sharply when we estimate the time and inocme specific hazard equation.
...
% complementary, devices in coping with C\&NS.
These effects are strong in the immediate following hour and fade away afterward. Furthermore, the effects are most pronounced when the cumulative income is approaching the typical daily average income, and become small and insignificant when the cumulative income is either too low or too hight.
...
% even after we account for recent earnings in \citet{thakral2018daily}'s fashion. 
...
\begin{table}
    ...
    \begin{tablenotes}
    	\parindent 1em%  %ChaoQin
		\small
        Each column is a separate regression. An observation is a booking. The sample includes only bookings immediately after which the drivers search for at least 3 minutes. The dependent variable for all columns is a binary variable ($Quit_{it}$) indicating if the booking is the last job of the shift. All regressions control for driver fixed effects, hour-day-of-week by building fixed effects, date fixed effects, cumulative hours and income, weather, market thickness, break time, time with passenger, duration and distance to pickup. ``Place trip 1" is a dummy variable indicating if the respective driver is the nearest the vacant driver to a C\&NS who end a completed booking at most 1 km and 3 minutes away from the C\&NS. ``Placebo trip 2" is a dummy variable indicating if respective driver end the booking within 200 m and 3 minutes from a C\&NS. Standard errors clustered by drivers in parentheses. $\textsuperscript{***}p < 0.01$, $\textsuperscript{**}p < 0.05$, $\textsuperscript{*}p < 0.10$. 
    \end{tablenotes}
    \label{tb:placebo}
\end{table}
...
\begin{table}[]
	...
	\begin{tablenotes}
		\parindent 1em%  %ChaoQin
		\small
	    Each row is a separate regression. An observation is a trip or a booking. The dependent variable for all row is a binary variable ($Quit_{it}$) indicating if the trip/booking is the last job of the shift. Unless indicated otherwise, the sample includes only bookings immediately after which the drivers search for at least 3 minutes. Unless indicated otherwise, all regressions control for driver fixed effects, hour-day of week by building fixed effects, date fixed effects,  cumulative hours and income, weather, market thickness, break time, passenger time, distance and duration to pickup. Unless indicated otherwise, the time-dependent hazard rate local weighted regressions use a bandwidth of 30 minutes and uniform weight; and the time-and-income-dependent hazard rate local weighted regressions use a bandwidth of 30 minutes and 20 SGD with uniform weight. Clustered standard errors in parentheses. $\textsuperscript{***}p < 0.01$, $\textsuperscript{**}p < 0.05$, $\textsuperscript{*}p < 0.10$.  
	\end{tablenotes}
