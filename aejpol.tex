...
\section{The mediating effects of cumulative income: Estimates using the set of bookings without immediate pickups}
Due to the data demanding nature of local weighted regression, estimating Equation \eqref{eq:2dlwr} using only the set of bookings without immediate pickups is challenging. We face a tradeoff between the precision of the estimates and the degree of control over how the hazard rate changes with time and income: larger bandwidth increase the sample size for each local weighted regression and hence its statistical power, but also decrease 
