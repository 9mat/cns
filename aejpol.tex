...
Due to the data demanding nature of local weighted regression, estimating Equation \eqref{eq:2dlwr} using only the set of bookings without immediate pickups is challenging. We face a tradeoff between the precision of the estimates and the degree of control over how the hazard rate changes with time and income when choosing the bandwidth: A larger bandwidth would allow for more variati
