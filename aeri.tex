...
\begin{table}[htb]
    ...
    \begin{tablenotes}
    	\parindent 1em%  %ChaoQin
		\small
        Each column is a separate regression. An observation is a booking. The sample includes only bookings immediately after which the drivers search for at least 3 minutes and at least two hours into the shift. The dependent variable for both columns is a binary variable (${CNS}t_{it}$) indicating if the booking is a CNS. All regressions control for driver fixed effects, hour-day-of-week by pickup building fixed effects, date fixed effects. Additional controls include cumulative hours and income, weather, market thickness, duration and distance to pickup. Standard errors clustered by drivers in parentheses.%$\textsuperscript{***}p < 0.01$, $\textsuperscript{**}p < 0.05$, $\textsuperscript{*}p < 0.10$. 
    \end{tablenotes}
\end{table}
