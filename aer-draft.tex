...
%         \begin{tabular}{l*{6}{S}}
		...
		\begin{tablenotes}
        The table reports summary statistics of shift-level data. Each observation is a shift---a shift is defined as a series of consecutive trips and bookings by the same driver less than 6 hours apart. 
		\end{tablenotes}	
% 	\end{threeparttable}
...
\end{tabular}
			...
			\begin{tablenotes}
            Each column is a separate regression. An observation is a trip or a booking. The dependent variable for all columns is a binary variable ($Quit_{it}$) indicating if the trip or booking is the last of the shift. Standard errors clustered by drivers in parentheses. $^* p<0.1, ^{**} p < 0.05, ^{***} p < 0.01$.
			\end{tablenotes}
%		\end{threeparttable}
	\end{table}
%\end{landscape}
