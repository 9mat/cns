...
\citet{camerer1997labor} find negative correlation between hours of work and daily wages among taxi drivers in New York city (NYC), a result that sharply contradicts neoclassical model of labor supply which predicts substitution of leisure for labor income when wage rises. \citet{kHoszegi2006model} propose a model of reference-dependent preference, theorizing that workers have a target level of income, which, together with loss aversion, motivates them to work harder when wage is low. The model neatly explains \citet{camerer1997labor}'s results while reconciling the opposing findings of positive wage elasticity in \citet{oettinger1999empirical} and \citet{fehr2007do}. \citet{kHoszegi2006model}'s key insight is that, assuming workers form their target rationally, reference dependence only matters if the variation in wages is unanticipated, because anticipated wage will change the income target in way that is consistent with neoclassical model. Thus, a good test of workers' reference dependence preference should look at the relationship between labor supply and unanticipated  shocks, not anticipated changes.
