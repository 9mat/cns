...
This paper proposes an alternative approach to test the neoclassical model and behavioral models of labor supply. We exploit the variation in external high-frequency small negative income shocks across workers and over time and examine how these shocks affect the workers' labor decision. We focus on the context of Singapore's taxi industry and the negative shocks that we use are booking cancellations and passenger no-shows, i.e., a situation in which a passenger makes a booking but fails to follow through with the trip, resulting in loss of time and millage from the driver's part without any earnings or compensation. These shocks are frequent enough to enable the use of individual fixed effects and a rich set of control variables and provide precise and reliable estimates of their effects on labor supply. We argue that this approach provide a much cleaner identification strategy for the test against neoclassical models than what the previous approaches do. The use of wage elasticity estimation for such purpose, most prominently in the seminal paper by \citet{camerer1997labor}, has been criticized by \citet{oettinger1999empirical} and \citet{farber2005tomorrow} on two fronts: (1) the use of realized earnings, an equilibrium outcome, as a proxy for wage rate is questionable, and (2) the construction of the wage rate is prone to measurement error and division bias\footnote{Wage is calculated by dividing total earnings by total hours of works, and any measurement error in hours of work will also be present in the resulting wage, leading to a spurious correlation between wage and hours of work.}.
