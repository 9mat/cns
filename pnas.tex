...
\newif\ifpagecount
% \pagecounttrue % comment out to for word counting

% no page number (to reduce AERI word count)
\ifpagecount
\pagestyle{empty}
\fi
...
%about not only street hail trips and taxi bookings, but also CNS. 
%
...
%the pickup location and around the booking time as exogenous source of variation affecting the CNS rate.
%The rationale and the results of this approach are discussed in Appendix \ref{apx:iv}. 
%Since the exogeneity test fails to reject the exogeneity of CNS, 
...
%individual earnings during a different time period in the shift, but CNS effects persist. Therefore,
...
% Because the source of income shocks that we use, CNS, is externally determined, it is unlikely that the 
...
margin in detail. % We first show that subsequent rate of earnings, an outcome that is increasing in effort, is positively affected by CNS, which indicates that CNS and income targets also have an effect on the intensive margin of labor supply. We subsequently examine other proxies for the intensive margin---breaks, idleness, driving speed, and bidding behavior---and observe similar patterns in all of these dimensions. 
...
impacts of CNS \citep{moore2001time,patrick2008reducing}, %norris2014empirical,feldman2014appointment}, 
...
Drivers rent vehicles from taxi operators at a daily rental of S\$70-120, depending on the model and age of the vehicle.
%, and typically falls between S\$70 and S\$120.
...
%For standard taxis, the base fare consists of (1) a flag-down fare of 3.2 to S\$3.9, (2) a variable distance rate of 22 to 25 cents for every 400 m in the first 10 km and for every 350 m thereafter, and (3) a waiting time fare of 22 to 25 cents for every 45 seconds. Time-based surcharges can be as high as 50\% of the meter fare between midnight and 6:00 a.m. Location surcharges range from 2 to S\$5. 
...
S\$3.3 during peak hours (6:00 p.m. to midnight every day and 6:00 a.m. to 9:30 a.m. on weekdays) and S\$2.3 during non-peak hours, which is added to the trip bill after the trip is completed. 
...
The booking fee is a nontrivial portion of the total fare. The typical booking fee (S\$3.3) equals 23\% of the average trip fare exclusive of the booking fee. Furthermore, there is no penalty for CNS.
% which makes street hail trips much more cost effective for passengers, except for the uncertainty of these trips. 
...
at any time and the driver is informed of % of the cancellation
...
%is completed, all information on the trip mentioned above. Bookings, with completed, failed, canceled, 
...
In total, 92\% of the original data survive this procedure. %Of these, 76.51\% are street hail trips, 21.51\% completed bookings, 484,448 (1.56\%) canceled bookings, and 129,363 (0.42\%) no-show bookings. 
An average street hail trip takes 16.02 minutes and earns the driver S\$12.85, while an average  
booking trip takes 20.12 minutes and earns S\$17.57, inclusive of booking fees.
...
We identified 2,122,256 shifts in total. An average shift lasts for 8.6 hours and earns the driver S\$200. 
...
our main analyses exclude all street hail trips as well as the subset of bookings where the drivers find a job within three minutes. In the language of experimental design, our control group is the decisions made by drivers who have no pickups within three minutes after completing a booking, and our treatment group is the decisions made by drivers who have no pickups within three minutes after ending a CNS. As we show in various robustness checks, our results are robust to other data selection, such as using the subsample of all bookings, or the full sample of both bookings and street hail trips.% results in similar estimates for the CNS effects.

To credibly rule out the influence of market conditions, we conduct a placebo test. We use vacant drivers who completed a booking within a small radius and time window from a CNS as the placebo subjects. Due to time and spatial proximity, these drivers are likely exposed to the same market environment as those who receive CNS.
% The placebo effect, which is the difference in subsequent labor decisions between the placebo group and the control group, would reflect the portion of the CNS effects that is  due to the correlation with market factors. 
Our analysis shows that the placebo effect is small and statistically insignificant, which rules out the correlation with market conditions as the explanation for the CNS effects, and suggests our identified CNS effects are causal.
...
The above patterns demonstrate %a significant negative correlation between CNS and the decision to stop work, as well as a significant positive correlation between CNS and subsequent earnings. In other words, 
that drivers are less likely to stop work and tend to earn more following CNS. %In subsequent sections, 
...
% We first look at labor supply and examine the decision to stop work after a CNS.
% We then study drivers' productivity after a CNS by examining the earnings in the immediate hour after each trip. 
...
% and unlikely %to be
% affected by driver behavior, and hence the variation in CNS is 
...
to continue working or end the shift by comparing the costs and benefits of continuing to those of stopping. 
% Following \citet{farber2005tomorrow,farber2015you}, w
We formulate this decision as a linear probability model,% 
% and assume a linear form for this function
\footnote{%
...
%by driver $i$ is canceled, and $0$ otherwise; and ${NS}_{it}=1$ if trip $t$ by driver $i$ is a 
...
The results %of the linear probability model 
are in Table \ref{tb:robustquit}. 
...
In addition, a S\$10 increase in cumulative earnings increases the hazard rate of stopping by 0.30 ppts (s.e. 0.02), and hence, a cancellation (no-show) has the same effect as having S\$41 (39) less in cumulative earnings.%, and a no-show S\$39 less.

\subsubsection{Time-dependent hazard of stopping}
The hazard rate of stopping changes substantially throughout the shift and failure to adequately control for the effects of cumulative hours can lead to spuriously significant estimates of the income effect due to the strong correlation between hours and income% \citep{thakral2018daily}
...
Equation \eqref{eq:lwr} allows for different hazard equations at different times in the shift, which means that the effects of %not only hours but also 
...
$w$ is the weighting function.%, which typically gives more weight to observation closer to $h$. A local weighted regression with a uniform weight
\footnote{A uniform weight with bandwidth $b$ sets $w(d) = 1$ if $|d|<b$ and $w(d) = 0$ otherwise. Another commonly used weight is the tricube weight: $w(d) = \left(1-(|d|/b)^3\right)^3$ if $|d|<b$, and $w(d) = 0$ otherwise.} 
...
% This not only improves the confidence in the validity of our specification, but also 
...
with working hours.
% , making it difficult to disentangle the two effects.  


% In summary, when regressing on different sets of controls and fixed effects, estimated CNS effects on
% drivers' decisions to stop work remain consistently negative, and the magnitudes vary
% from 17\% to 30\% reduction in the hazard rate of stopping. This negative relationship between CNS and the decision to stop work cannot
% be entirely explained by time effects, location effects, drivers' characteristics, fatigue, or passengers' 
% selective CNS decisions. Therefore, it is likely to reflect a causal relationship between CNS and 
...
The U-shaped pattern of the moderating effect of cumulative income can be observed clearly at all the five time points. The CNS effects are strongest and most significant when the income is in the range of S\$150-250. This turning point slightly shifts to the right to around S\$250 once the cumulative hours past 9 hours. The CNS effects decrease in magnitude and become insignificant when the cumulative income is either too low or too high. This is consistent with income-targeting behavior: When cumulative income is too low, there is a very low chance for the driver to reach the target, and hence it reduces the incentive to work more when drivers encounter adverse earnings shocks. On the other hand, when income is too high, the driver has likely exceeded the target, and extra earnings do not change the marginal utility of income. We also run these local weighted regression models with the subset of data used in the main analysis and obtained similar but less precise results (Appendix \ref{apx:CI}).
...
. The first five columns in Table \ref{tb:robustwage} report the estimates for various sets of fixed effects and control variables. The estimated effects of CNS on the earnings in the next hour are positive and significant across all five specifications. The magnitudes of the effects are also robust to the inclusion of the additional sets of control variables: A cancellation is associated with an average increase between S\$0.98 (s.e. 0.05, Specification (5)) and S\$1.59 (s.e. 0.05, Specification (1)) in the earnings, and a no-show with an increase between S\$0.76 (s.e. 0.08, Specification (3)) to S\$1.36 (s.e. 0.08, Specification (1)). The only notable change in the estimated coefficients is with the inclusion of the break time and the time with passenger in Specification (3), which seems to account for 30\% of the cancellation effect and 40\% the no-show effect in Specification (2). While fatigue does not seem to affect the decision to stop---accounting for only 10\% of the CNS effects on the hazard rate of stopping in Table \ref{tb:robustquit}---it seems to have a meaningful effect on the subsequent productivity. However, there is still 70\% (60\%) of the cancellation (no-show) effect %and 60\% of the no-show effect 
on earnings that cannot be explained by fatigue, or by the additional sets of control variables in Specifications (4) and (5). The average earnings increase of S\$0.98 in an hour after a cancellation %, according to Specification (5) with the full set of controls,
represents a 4.5\% increase from the baseline hourly earnings of S\$21.6, while the no-show effect, S\$0.82, represents a 3.8\% increase. The increase in the subsequent earnings suggests that the drivers choose to work not only  longer but also more diligently after encountering the negative events of CNS.


To see how the effects of CNS on earnings change over time, we reestimate Equation \eqref{eq:outcomes} changing the dependent variable to the earnings in different 30-minute intervals after the end of a trip. Columns (6)-(7) in Table \ref{tb:robustwage} report the estimation results for the earnings in the two 30-minute intervals in the second hour after the trip. The CNS effects decay quickly over time. The cancellation effect on the earnings in the interval 60 to 90 minutes after the trip is only an increase of 17 cents (s.e. 4.2), or a 1.5\% increase from the average half-hourly earnings of S\$11.6. It becomes statistically insignificant after 90 minutes. The no-show effect decays even faster and becomes statistically insignificant after one hour.
...
as much of an unexpected shock as a more recent CNS does. %As a result, 
...
Table \ref{tb:earningsbreakdown} shows that after CNS, taxi drivers adjust their labor and productivity along multiple margins. In the immediate hour following CNS, they reduce the break time by 15\% (16\%) %or 0.82 minutes (
for cancellation (no-show) %and by 16\% or 0.86 minutes (for no-show
 from an average break time of 5.3 minutes, increase the speed when with passenger by more than 10\% to complete the jobs faster %(an increase of 4 km/h (s.e. 0.19) and 4.5 km/h (s.e. 0.32) from the average speed 35 km/h),
...
The effects of CNS on extensive and intensive margins combined lead to S\$3.45 (s.e. 0.07) increase in shift income after a cancellation and S\$0.68 (s.e. 0.11) increase after a no-show (Appendix Table \ref{tb:overcompensate}).
...
% to work more diligently after a CNS. This difference may be due to the nature of the work---e.g., the 
% driver's work is flexible and independent---or the severity of the adverse events, since a CNS only costs
...
% CNS may have an immediate negative effect on drivers' earnings due to the 
...
% income is not obvious: Can drivers make up for all the loss that CNS incur, and do they ever 
% overcompensate for the negative effects? Table \ref{tb:overcompensate} reports the relationship between 
% CNS and total shift income.
% The estimates suggest that taxi drivers tend to overcompensate for cancellations,
% and total income tends to be S\$3.4 higher in shifts with some cancellations.
...
% Both of these tests are based on the following rationale: Due to location and time proximity, the drivers with the placebo CNS are subject to the same market conditions as the drivers with the actual CNS, and thus, the effects of the placebo CNS will capture any spurious correlation between CNS and labor supply due to market demand and supply. 
Both placebo tests %effects are estimated to 
have negligible, statistically insignificant effects on either the decisions to stop or subsequent earnings (Appendix Table \ref{tb:placebotest}). As a result, we can rule out unobserved market conditions as an explanation for our results.
...
% that some drivers may be more familiar with certain regions than others. All of the estimates for CNS effects
% on stopping work in these three columns are similar to results in previous sections. Row (12) estimates the CNS effects for the trips and bookings from CBD, an area that must be very familiar to all the drivers and in which we expect little heterogeneity in
...
% drivers' ability, experience, or plans do not explain CNS effects on labor supply
...
% \paragraph{Serial correlation in CNS} CNS may have correlation with itself over time; if so, encounters
% with CNS may inform drivers about more (fewer) CNS in the remaining part of their shifts if the serial 
% correlation is positive (negative). We observe a positive and small 
% serial correlation 0f 0.22 in hourly rate of CNS after having partialled out hour of day, day of week, and date 
% fixed effects. More importantly, most of 
% the autocorrelation comes from the 5 am to 2 pm period, during which CNS effects are small, while 
% autocorrelations from 2 pm to 10 pm and 10 pm to 5 am, when most stopping decisions are made for day-shift
% and night-shift drivers, and also when CNS effects are the strongest, are much negigible and insignificant---0.05
% (s.e. 0.04) and -0.02 (s.e. 0.05), respectively. Positive serial correlation in CNS would actually strengthen
% the implications of our results, because knowing that there are more CNS to come, drivers' motivation must 
...
on the dynamics of the reference point. The fact that the timing of CNS matters, with the effects fading away
over time, suggests that the reference point is adaptive to new events. The magnitude of the CNS effects---a
one third reduction in the hazard rate of stopping, equivalent to the effect of S\$41 less in realized 
...
% 		\begin{tabular}{lccccccc}
        \begin{tabularx}{\textwidth}{l@{\extracolsep{\fill}}*{7}{S[table-format = 0.4, table-space-text-pre={**}, table-space-text-post={-**}]}}
			...
			\addlinespace
			S\$10  more of earnings         &                         &                         &                         &                         & 0.0030\sym{***}         &                               &                               \\
			                                &                         &                         &                         &                         & (0.0002)                &                               &                         \\ 
		   \midrule
	...
	\begin{tablenotes}
	\small
        Each column is a separate regression. An observation is a booking. The sample includes only bookings immediately after which the drivers search for at least 3 minutes. The dependent variable for all columns is a binary indicator if the booking is the last job of the shift. Column (6) and (7) use local weighted regression with a bandwidth of 30 minutes and uniform weights. All regressions include driver fixed effects, hour\(\times\)day-of-week\(\times\)building fixed effects, date fixed effects, and a cubic function of the cumulative hours. Control variables for \textit{Weather} are temperature, relative humidity, PM 2.5, rain duration and rainfall. Control variables for \textit{Market thickness} are the number of bookings and street hail trips within 500 m and 5 minutes, within 1,000 m and 5 minutes, and within the same zone and clock hour, and the number of vacant vehicle within 500 m and 5 minutes. Control variables for \textit{Break and passenger time} are the number of break minutes and the number of minutes with passenger from shift start, and in the last hour. \textit{On-call time and distance} are the duration and distance the drivers travel to pick up passengers. \(g(y_{it})\) is a cubic function of the cumulative earnings. Standard errors clustered by drivers in parentheses.%$\textsuperscript{***}p < 0.01$, $\textsuperscript{**}p < 0.05$, $\textsuperscript{*}p < 0.10$. 
	\end{tablenotes}
\end{table}
...
\begin{table}[]
	...
	\centering
	{
        ...
        \begin{tabularx}{\textwidth}{l@{\extracolsep{\fill}}*{7}{S[table-format = 0.4, table-space-text-pre={**}, table-space-text-post={-**}]}}
			\toprule
			\toprule
			&\multicolumn{5}{c}{Earnings in next hour (S\$)} &\multicolumn{1}{>{\centering\arraybackslash}m{1.5cm}}{Earnings 60 to 90 mins after trip (S\$)}&\multicolumn{1}{>{\centering\arraybackslash}m{1.5cm}}{Earnings 90 to 120 mins after trip (S\$)}\\
			\cmidrule(lr){2-6}  \cmidrule(lr){7-7} \cmidrule(lr){8-8}                             
			&\multicolumn{1}{c}{(1)}         &\multicolumn{1}{c}{(2)}         &\multicolumn{1}{c}{(3)}         &\multicolumn{1}{c}{(4)}         &\multicolumn{1}{c}{(5)}         &\multicolumn{1}{c}{(6)} &\multicolumn{1}{c}{(7)} \\
			\midrule
			...
			\addlinespace
			S\$10 more of earnings&                     &                     &                     &                     &     -0.1218\sym{***}         & & \\
			&                     &                     &                     &                     &    (0.0099)         & & \\
			\midrule
	...
	\begin{tablenotes}
		\small
		Each column is a separate regression. An observation is a booking. The sample includes only bookings immediately after which the drivers search for at least 3 minutes. The dependent variable for the first five columns is the total earnings earned in 60 minutes immediately following the booking. The dependent variable for Column (6) and (7) is the total earnings earned in the interval 60-90 minutes and 90-120 minutes from the end of the booking. All regressions include driver fixed effects, hour\(\times\)day-of-week\(\times\)building fixed effects, date fixed effects, and a cubic function of the cumulative hours. Control variables for \textit{Weather} are temperature, relative humidity, PM 2.5, rain duration and rainfall. Control variables for \textit{Market thickness} are the number of bookings and street hail trips within 500 m and 5 minutes, within 1,000 m and 5 minutes, and within the same zone and clock hour, and the number of vacant vehicle within 500 m and 5 minutes. Control variables for \textit{Break and passenger time} are the number of break minutes and the number of minutes with passenger from shift start, and in the last hour. \textit{On-call time and distance} are the duration and distance the drivers travel to pick up passengers. \(g(y_{it})\) is a cubic function of the cumulative earnings. Standard errors clustered by drivers in parentheses.%$\textsuperscript{***}p < 0.01$, $\textsuperscript{**}p < 0.05$, $\textsuperscript{*}p < 0.10$. 
	\end{tablenotes}
\end{table}
...
%     \toprule
%     \toprule
%     			&\multicolumn{5}{c}{\textit{Dependent variable}: Earnings in next hour (S\$)} \\
%     			\cmidrule{2-6}
%                 &\multicolumn{1}{c}{(1)}         &\multicolumn{1}{c}{(2)}         &\multicolumn{1}{c}{(3)}         &\multicolumn{1}{c}{(4)}         &\multicolumn{1}{c}{(5)}         \\
%     \midrule
...
\begin{figure}[htb]
	{\centering
		\caption{Stopping decisions and earnings in the hours after CNS} %ChaoQin
...
% \begin{figure}[htb]
	{\centering
		\caption{Normalized CNS effects over time and at different income levels (full sample estimation)} %ChaoQin
		\includegraphics[width=0.75\textwidth]{./fg/cnsmfxulwr80to100normalized.pdf}
		\label{fg:2dulwr}
	}
	\begin{figurenotes}
	\small\ Each dot in the graph represents an estimate from a separate local weighted regression used to estimate the hazard of stopping at a specific time in the shift and a specific income level. The number in the title of each panel is the number of hours since the start of the shift at which the hazard is evaluated. The horizontal axis indicates the income level at which the hazard is evaluated. All local weighted regressions use a bandwidth of 30 minutes for time and S\$20 for income, and uniform weight to weigh the observations. Normalized CNS effects are calculated by dividing the estimated CNS coefficients from the regressions by the local weighted hazard rates at the correspoding time in the shift and income level. 
	\end{figurenotes}
\end{figure}
...
% \begin{centering}
% \underline{\huge{\textbf{For Online Publication}}}\\
% \end{centering}
...
\begin{itemize}[noitemsep,nolistsep]
	...
	\item observations with trip fare of value less than S\$3.2 or more than S\$100 (0.13\%)%\footnote{S\$100 is the 99.9725 percentile of the original trip fare distribution.}
	\item observations with trip distance shorter than 100 m or longer than 100 km (1.19\%)%\footnote{S\$100 is the 99.9725 percentile of the original trip fare distribution.}
	\item observations in a shift with shift duration less than 1 hour or more than 24 hours (1.82\%)
	\item observations in a shift with shift earnings less than S\$10 or more than S\$1,000 (0.01\%)%\footnote{S\$1000 is the 99.9725 percentile of the original shift income distribution.}
...
\section{CNS effects on total shift income}
\label{sec:shiftincome}
...
% 			\multicolumn{3}{l}{Panel C: Effects of CNS on shift income}\\			
			\toprule
			                    &\multicolumn{2}{c}{Shift income (S\$)}     \\
			                    \cmidrule{2-3}
...
\section{Placebo tests, Robustness check and other results}

% % \begin{table}
%     \centering
%     \footnotesize
%     \caption{Placebo Test}
%     \label{tb:placebotest}
%     \def\sym#1{}%\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}
...
%     	\parindent 1em%  %ChaoQin
% 		\small
%         Each column is a separate regression. An observation is a booking. The sample includes only bookings immediately after which the drivers search for at least 3 minutes. The dependent variable for all columns is a binary variable ($Quit_{it}$) indicating if the booking is the last job of the shift. All regressions control for driver fixed effects, hour-day-of-week by building fixed effects, date fixed effects, cumulative hours and income, weather, market thickness, break time, time with passenger, duration and distance to pickup. ``Placebo booking 1" is a dummy variable indicating if the respective driver is the nearest vacant driver to a CNS who ends a completed booking at most 1 km and 3 minutes away from the CNS. ``Placebo booking 2" is a dummy variable indicating if respective driver ends the booking within 200 m and 3 minutes from a CNS. Standard errors clustered by drivers in parentheses.%$\textsuperscript{***}p < 0.01$, $\textsuperscript{**}p < 0.05$, $\textsuperscript{*}p < 0.10$. 
%     \end{tablenotes}
% \end{table}
...
\begin{table}[htb]
    ...
    \begin{tabularx}{\textwidth}{l@{\extracolsep{\fill}}*{4}{c}} 
        \toprule
        \toprule
                            &\multicolumn{2}{c}{Stopping work (dummy)}         &\multicolumn{2}{c}{Earnings in next hour (S\$)}\\
                            \cmidrule(lr){2-3}\cmidrule(lr){4-5}
                            &\multicolumn{1}{c}{(1)}         &\multicolumn{1}{c}{(2)}         &\multicolumn{1}{c}{(3)}         &\multicolumn{1}{c}{(4)}         \\
        \midrule
        Placebo trip 1 (dummy)&    -0.00003         &                     &     0.07200\sym{*}  &                     \\
                            &    (0.0007)         &                     &    (0.0371)         &                     \\
        \addlinespace
        Placebo trip 2 (dummy)&                     &     0.00026         &                     &    -0.04098         \\
                            &                     &    (0.0007)         &                     &    (0.0355)         \\
        \midrule
        Mean hazard rate    &      0.0534         &      0.0534         &                     &                     \\
        Observations        &\num{3743047}         &\num{3743047}         &\num{3262162}         &\num{3262162}         \\
        Drivers             & \num{31958}         & \num{31958}         & \num{31757}         & \num{31757}         \\
        \(R^2\)             &      0.3374         &      0.3374         &      0.4040         &      0.4041         \\
        \bottomrule
    \end{tabularx}
    \begin{tablenotes}
    	\parindent 1em%  %ChaoQin
		\small
        Each column is a separate regression. An observation is a booking. The sample includes only bookings immediately after which the drivers search for at least 3 minutes. The dependent variable for Column (1) and (2) is a binary variable ($Quit_{it}$) indicating if the booking is the last job of the shift. The dependent variable for Column (3) and (4) is the total earnings in the hour following the booking. All regressions control for driver fixed effects, hour-day-of-week by building fixed effects, date fixed effects, cumulative hours and income, weather, market thickness, break time, time with passenger, duration and distance to pickup. ``Placebo booking 1" is a dummy variable indicating if the respective driver is the nearest vacant driver to a CNS who ends a completed booking at most 1 km and 3 minutes away from the CNS. ``Placebo booking 2" is a dummy variable indicating if respective driver ends the booking within 200 m and 3 minutes from a CNS. Standard errors clustered by drivers in parentheses.%$\textsuperscript{***}p < 0.01$, $\textsuperscript{**}p < 0.05$, $\textsuperscript{*}p < 0.10$. 
    \end{tablenotes}
\end{table}
...
\begin{table}[htb]
    \centering
    \footnotesize
    \caption{Past earnings do not predict CNS}
    \label{tb:pastearnings}
    \def\sym#1{}%\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}
    \begin{tabularx}{\textwidth}{l@{\extracolsep{\fill}}*{2}{c}} 
        \toprule
        \toprule
                    &\multicolumn{2}{c}{CNS (dummy)}         \\
                    \cmidrule{2-3}
                    &\multicolumn{1}{c}{(1)}         &\multicolumn{1}{c}{(2)}         \\
        \midrule
        Earnings in the last 30 minutes (S\$)&   -0.000042         &    0.000034         \\
                    &  (0.000040)         &  (0.000038)         \\
        \addlinespace
        Earnings in the last hour (S\$)&    0.000031         &    0.000012         \\
                    &  (0.000026)         &  (0.000025)         \\
        \addlinespace
        Driver, date, hour\(\times\)day-of-week\(\times\)building FE&         {Y}         &         {Y}         \\
    ...
    \begin{tablenotes}
    	\parindent 1em%  %ChaoQin
		\small
        Each column is a separate regression. An observation is a booking. The sample includes only bookings immediately after which the drivers search for at least 3 minutes and at least two hours into the shift. The dependent variable for both columns is a binary variable (${CNS}t_{it}$) indicating if the booking is a CNS. All regressions control for driver fixed effects, hour-day-of-week by pickup building fixed effects, and date fixed effects. Additional controls include cumulative hours, weather, market thickness, duration and distance to pickup. Standard errors in parentheses.%$\textsuperscript{***}p < 0.01$, $\textsuperscript{**}p < 0.05$, $\textsuperscript{*}p < 0.10$. 
    \end{tablenotes}
\end{table}
...
\begin{table}[htb]
    ...
    \begin{tabularx}{\textwidth}{ll@{\extracolsep{\fill}}*{4}{c}}
    	...
    	(17) & Hazard rate at 8.5 hours and S\$200, S\$20 bandwidth & -0.014 (0.010) & -0.021 (0.017) & 0.134 \\
    	(18) & Hazard rate at 8.5 hours and S\$200, S\$10 bandwidth & -0.001 (0.017) & 0.012 (0.027) & 0.125 \\
    	(19) & Hazard rate at 8.5 hours and S\$200, tricube weight & -0.028\sym{*} (0.013) & -0.022 (0.023) & 0.136 \\
    	\bottomrule
    \end{tabularx}%
    \label{tb:immepickup}
	\begin{tablenotes}
...
\begin{table}
    ...
    \begin{tablenotes}
    	\parindent 1em%  %ChaoQin
		\small
        Each column is a separate regression. Data: time series of hourly nationwide CNS rate (number of CNS divided by number of bookings in each hour). Dependent variable: hourly nationwide CNS rate. The top one percentile CNS rates are dropped from all regressions. Estimated by OLS with date fixed effects and hour-of-day\(\times\)day-of-week fixed effects. Column (1) uses the full sample. Column (2) uses only observations from hours from 5 am to 2 pm each day, Column (3) uses 2 pm to 10 pm, and Column (4) uses 10 pm to 5 am, respectively. Standard errors in parentheses.%$\textsuperscript{***}p < 0.01$, $\textsuperscript{**}p < 0.05$, $\textsuperscript{*}p < 0.10$. 
    \end{tablenotes}
    \label{tb:ccar}
\end{table}
